...
```js
import { Web8Client } from 'https://esm.sh/@oasisomniverse/web8-api@1.0.3';
const client = new Web8Client({ baseUrl: 'https://api.web8.oasisomniverse.one' });
```
...
const client = new Web8Client({ baseUrl: '...', fetchImpl: fetch });
```
