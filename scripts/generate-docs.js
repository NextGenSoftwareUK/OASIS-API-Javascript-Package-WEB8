...
function parseModuleFile(filePath) {
  ...
  const sourceFileMatch = src.match(/source: WEB8 Mesh WebAPI (\S+)\)/);
...
for (const file of files) {
  ...
  const content = `# ${moduleName} — \`web8.${clientProp}\`

Source controller: [\`${sourceFile}\`](https://github.com/NextGenSoftwareUK/OASIS2/blob/main/WEB8/NextGenSoftware.OASIS.Web8.WebAPI/Controllers/${sourceFile})
...
const { isError, message, result } = await web8.${clientProp}.${exampleOp.jsName}(${exampleArgs});
...
const indexRows = moduleSummaries
  .map((m) => `| [\`web8.${m.clientProp}\`](modules/${m.moduleName}.md) | \`${m.routePrefix}\` | ${m.opCount} |`)
  .join('\n');

const totalOps = moduleSummaries.reduce((n, m) => n + m.opCount, 0);

const indexContent = `# WEB8 Galactic Mesh & Protocol Bridge API — JavaScript SDK Reference
